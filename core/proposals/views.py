...
@login_required
def admin_tracking_dashboard(request):
    if not request.user.is_admin_office:
        messages.error(request, "Access denied.")
        return redirect('users:dashboard')
    ...
    base_qs = Proposal.objects.filter(
        reviewing_office=admin_profile.office_type
    ).exclude(status=Proposal.Status.DRAFT).select_related('organization')

    counts = {
        'submitted': base_qs.filter(status=Proposal.Status.SUBMITTED).count(),
        'under_review': base_qs.filter(status=Proposal.Status.UNDER_REVIEW).count(),
        'approved': base_qs.filter(status=Proposal.Status.APPROVED).count(),
        'rejected': base_qs.filter(status=Proposal.Status.REJECTED).count(),
        'total': base_qs.count(),
    }
    counts['pending'] = counts['submitted'] + counts['under_review']

    status_filter = request.GET.get('status', '').upper()
    proposals = base_qs.filter(status=status_filter) if status_filter else base_qs
    proposals = proposals.order_by('-updated_at')

    return render(request, 'proposals/admin_tracking_dashboard.html', {
        'counts': counts,
        'proposals': proposals,
...
def public_proposals(request):
    ...
    if office_filter:
        proposals = proposals.filter(reviewing_office=office_filter)
    if status_filter:
        proposals = proposals.filter(status=status_filter)

    office_choices = [('CFMO', 'CFMO'), ('OSA', 'OSA'), ('CSMO', 'CSMO'), ('OTHER', 'Other')]
    status_choices = [
    ...
    return render(request, 'proposals/public_proposals.html', {
        'proposals': proposals,
